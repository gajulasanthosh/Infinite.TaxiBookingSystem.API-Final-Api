...
namespace Infinite.TaxiBookingSystem.API.Repositories
{
    public class EmployeeRosterRepository : IRepository<EmployeeRoster>,IEmployeeRosterRepository, IGetRepository<EmployeeRosterDto>
    {
        ...
        public async Task<IEnumerable<EmployeeRosterDto>> SearchByEmp(string employeeName)
        {
            if (!string.IsNullOrWhiteSpace(employeeName))
            {
                var employeeswithname = await _Context.EmployeeRosters.Include(x => x.Employee).Select(x => new EmployeeRosterDto
                {
                    Id = x.Id,
                    EmployeeId = x.EmployeeId,
                    FromDate = x.FromDate,
...
                //var employeeRoster = await _Context.EmployeeRosters.Include(x => x.Employee).Where(x => x.Employee.EmployeeName.Contains(employeeName)).ToListAsync();
                var employeeRoster = employeeswithname.Where(x => x.EmployeeName.Contains(employeeName)).ToList();
                return employeeRoster;
            }
            return null;
        }
